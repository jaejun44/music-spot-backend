...
사용자가 직접 등록한 연습실도 같은 `Room` 테이블에 들어간다. 구분은 `ownerId`다.
...
| `GET /api/rooms/:id` · `POST /api/rooms` (인증)                 | `{ room }`                                                                                       |
| `DELETE /api/rooms/:id` (인증)                                  | `{ message }`                                                                                    |
| `GET /api/posts?page&size`                                      | `{ posts, total, page, size, totalPages, hasNext }` — 각 글에 `likeCount`·`commentCount`·`liked` |
| `POST` · `PATCH` `/api/posts/:id` (인증) · `GET /api/posts/:id` | `{ post }` — 상세는 `comments` 포함                                                              |
| `POST /api/posts/:id/like` (인증)                               | `{ liked, likeCount }` — 토글                                                                    |
| `POST /api/posts/:id/comments` (인증)                           | `{ comment }`                                                                                    |
| `DELETE /api/posts/:id` · `/api/comments/:id` (인증)            | `{ message }`                                                                                    |
| 에러                                                            | `{ message }`                                                                                    |

게시글·댓글의 작성자는 **`toPublicPost()` / `toPublicComment()`를 거쳐** `{ id, username }`만 나간다. `include: { author: true }` 결과를 그대로 `res.json()` 하면 비밀번호 해시가 샌다.

커뮤니티 목록·상세는 **선택적 인증**(`createOptionalAuthMiddleware`)을 쓴다. 토큰이 없거나 상해도 막지 않고 통과시키되, 있으면 `liked`(내가 좋아요를 눌렀는지)를 채운다. 여기서 막으면 비로그인 사용자가 글을 아예 못 본다.
