...
**요청** — 지역(`sido`·`gungu`)은 보내지 않습니다. 서버가 주소에서 뽑습니다.

```json
{
  "name": "우리 밴드 합주실",
  "address": "서울 마포구 동교동 155-20 지하 1층",
  "category": "합주실",
  "pricePerHour": 15000,
  "phone": "010-1234-5678",
  "hours": "10~24시",
...
등록한 연습실은 검색 결과 **맨 앞**에 옵니다. 후기가 없어 후기순으로만 정렬하면 크롤링 576곳 뒤에 묻히기 때문입니다.

| 상태  | 상황                       | 메시지                                                             |
| ----- | -------------------------- | ------------------------------------------------------------------ |
| `400` | 지역을 알아볼 수 없는 주소 | `"주소는 시/도와 시·군·구로 시작해야 합니다. (예: 서울 마포구 …)"` |
| `401` | 토큰 없음·만료             | `"로그인이 필요합니다."`                                           |

---

## DELETE `/api/rooms/:id` — 연습실 삭제 🔒

**등록자 본인만** 지울 수 있습니다. 크롤링으로 들어온 576곳은 `ownerId`가 없어 **아무도 지울 수 없습니다.**

**응답 `200 OK`** — `{ "message": "연습실을 삭제했습니다." }`

| 상태  | 상황                      | 메시지                                       |
| ----- | ------------------------- | -------------------------------------------- |
| `401` | 토큰 없음·만료            | `"로그인이 필요합니다."`                     |
| `403` | 남의 연습실 / 크롤링 시드 | `"내가 등록한 연습실만 삭제할 수 있습니다."` |
