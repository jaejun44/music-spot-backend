...
- **배포된 API**: `https://music-spot-backend.onrender.com`
...
model Room {
  id           Int      @id @default(autoincrement())
  name         String   // "숨 음악연습실"
  location     String   // "서울 노량진동"
  category     String   // "합주룸"
  pricePerHour Int      // 7000
  createdAt    DateTime @default(now())
}
```
...
Base URL — 로컬 `http://localhost:3000` / 배포 `https://music-spot-backend.onrender.com`
  ...
  "rooms": [
    {
      ...
      "category": "합주룸",
      "pricePerHour": 7000,
      "createdAt": "2026-07-13T07:00:24.468Z"
    }
  ]
}
```

등록된 연습실이 없으면 빈 배열 `{ "rooms": [] }`을 돌려줍니다. (에러가 아닙니다)
...
**응답 `200 OK`**
```json
{ "room": { "id": 1, "name": "숨 음악연습실", "location": "서울 노량진동", "category": "합주룸", "pricePerHour": 7000, "createdAt": "..." } }
```
...
서비스·도메인·인증 미들웨어에 대한 단위 테스트 **52개**가 있습니다. DB 없이 돌아갑니다 (가짜 의존성 주입).
...
| `room.service.test.ts` | 목록/상세 조회, 빈 목록, 없는 id 404 |
